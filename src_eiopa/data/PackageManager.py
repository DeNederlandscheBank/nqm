...
from src.data import FileSource, Locale, XmlUtil, UrlUtil
...
class Taxonomies:
    ...
    def __init__(self, path):
        ...
        if self.config is None:
            self.config = {  # savable/reloadable plug in configuration
            ...
            for j in range(i-1, -1, -1):
                ...
                if (_packageURL != _packageURL2 and _prefix and _prefix2 and 
                    ...
                    if _url1 == _url2: # use full file names
                        ...
                        _url2 = _packageURL2
                    if _prefix == _prefix2:
                        print("Packages overlap the same rewrite start string " + _prefix)
                    else:
            ...
            if prefixValue and replaceValue is not None:
                if prefixValue not in remappings:
                    ...
                    if replaceValue: # neither None nor ''
                        if not UrlUtil.isAbsolute(replaceValue):
...
def packageInfo(URL, reload=False, packageManifestName=None, errors=[]):
    ...
    if packageFilename:
        filesource = None
        try:
            filesource = FileSource.openFileSource(packageFilename)
